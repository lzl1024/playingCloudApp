...
// <<< Don't Change
    ...
    public static class TitleCountMap extends Mapper<Object, Text, Text, IntWritable> {
        ...
        StringTokenizer st = new StringTokenizer(value.toString(), this.delimiters);
                Map<String, Integer> lineCounters = new HashMap<String, Integer>();
                ...
                while (st.hasMoreTokens()) {
                        ...
                        if (!lineCounters.containsKey(word)) {
                                lineCounters.put(word, 1);
                        } else {
                                lineCounters.put(word, lineCounters.get(word) + 1);
                        }
                }

            for (Map.Entry<String, Integer> e: lineCounters.entrySet()) {
    ...
    public static class TopTitlesReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
	    ...
	    TreeSet<Pair<Integer, String>> all = new TreeSet<Pair<Integer, String>>();
            for (TextArrayWritable t : values) {
		String[] line = t.toStrings();
		Pair<Integer, String> p = Pair.of(Integer.parseInt(line[1]), line[0]);
		all.add(p);
	    }

	    ArrayList<Pair<Integer, String>> result = new ArrayList<Pair<Integer, String>>(all);
            for (int i = result.size()-1; i >= result.size()-N; i--) {
		context.write(new Text(result.get(i).second), new IntWritable(result.get(i).first));
	    }
        }
    }
